 ...
 *
 * <p>Without this the correlation id is a field on log lines nobody writes. A successful request
 ...
 *
 * <p>The id is not passed as an argument: it is in the MDC, and the ECS encoder puts every MDC
 * entry on the line as a field. That is what makes it searchable rather than a string somebody has
 * to grep out of a message.
 *
 * <p>Ordered after {@link CorrelationIdFilter} so the id is already there, and it times the whole
 * chain below it rather than the handler alone — which is the number a caller experiences.
 */
@Component
@Order(3)
