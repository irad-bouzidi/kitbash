...
class WidgetApiIntegrationTest : PostgresTestBase() {
    ...
    fun createReadRestock() {
        val created =
            http.postForEntity(
                "/api/widgets",
                CreateWidgetRequest("flux capacitor", 2),
        ...
        val restocked =
            http.postForEntity("/api/widgets/{id}/restock", RestockBody(5), WidgetResponse::class.java, id)
        assertThat(restocked.body).isNotNull
        assertThat(restocked.body!!.quantity).isEqualTo(7)

        assertThat(http.getForObject("/api/widgets/{id}", WidgetResponse::class.java, id).name)
    ...
    fun duplicateNameIsConflict() {
        http.postForEntity("/api/widgets", CreateWidgetRequest("flux capacitor", 1), WidgetResponse::class.java)

        val conflict =
            http.postForEntity(
                "/api/widgets",
                CreateWidgetRequest("flux capacitor", 1),
