...
- `domain/` has no framework on it at all — no Spring, no Jakarta Persistence, no Jackson — and the
  database row it maps to is a separate class in `adapter/outbound/persistence`;
- the application declares the interfaces it needs (`application/port/outbound`) instead of
  importing the code that satisfies them, so the dependency points inwards;
- the web layer depends on a port (`application/port/inbound`), not on a service class.

`ArchitectureTest` fails the build on each of those, so none of it is a convention. Adding a JPA
annotation to the domain is a red build with a sentence explaining why, not a review comment
somebody may or may not leave.
